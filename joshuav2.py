...
class rulebook:
    ...
    #When is B canceled by head to head bounce (A --> <-- B  or  A --> B or A <-- B)
    #           |  A move    | A move via  | A convoy | A sup mov  | A sup hol |
    #B move     |  never     | never       |          |            |           |
    #B move via |  never     | never       |          |            |           |
    #B convoy   |  A* > all  | A* > all    |          |            |           |
    #B sup mov  |  always*   |             |          |            |           |
    #B sup hol  |  always*   |             |          |            |           |
    #B hold     |  always*   |             |          |            |           |

    #When is B canceled by directly opposed order A (A <--> B)                |
    #           |  A move   | A move via | A convoy | A sup mov  | A sup hol  |
    #B move     |  A >= B   |  A >= B    |  A >= B  |  A >= B    |  A >= B    |
    #B move via |  A >= B   |  A >= B    |  A >= B  |  A >= B    |            |
    #B convoy   |  A* > all |            |  never   | any* > [B] | any* > [B] |
    #B sup mov  |  A* > all |  A* > all  |  never   |   never    |   never    |
    #B sup hol  |  always   |  A* > all  |  never   |   never    |   never    |
    #B hold     |  never    |  never     |  never   |   never    |   never    |
    ...
    def __init__(t):
        ...
        x = parse('''matching_supports 1901
outer:
* A SUP * B to C
* A SUP * B to C
* A SUP * B HOL
...
* B CON * C to D
* B CON * C to E via * D
''', fres = [], seasons=['matching_supports', 'matching_via'], phases=['unresolved','canceled'])

        t.sup_rules = (x[0][0].ords['inner:'], x[0][0].ords['outer:'])

    def sup_minmax(t, ord_, ignore = ''):
        return 1, 1 #temp

    def has_more_sup(t, any_A = [] , all_B = [], or_equal = False, ignore = ''):
        mina, maxa = 0, 0
        for a in any_A: 
            x,y = t.sup_minmax(a, ignore)
            mina = max(mina, x)
            maxa = max(maxa, y)
        new_B = []
        minb, maxb = 0,0
        for b in all_B:
            x,y = t.sup_minmax(b, ignore)
            if x > maxa: return False #nothing in A can be greater than all B, because this B is bigger than all A
            if y > mina:
                new_B.append(b)
                minb = max(minb, x)
                maxb = max(maxb, y)
        if mina > maxb: return True #nothing in B can be greater than something in A
        new_A = []
        for a in any_A:
            x,y = t.sup_minmax(a, ignore)
            if x >= minb: new_A.append(a)
        #ok now what? 
...
class unit:
    ...
    def __and__(t,o):
        return streq(t.player, o.player) and streq(t.ut,o.ut)
...
class territory:
    ...
    def get_u(t, to = '*',autofill = False):
        if to[0] == '~': return t.disl.get_u(to)
        if to[0] == '_': return t.perse.ut.get_u(to)
        if to == '*': return t.occu.get_u(to)
        return t.get_c(to.strip('*')).get_u(to)

    def add_o(t, o, to='*', replace = False):
        u = t.get_u(to, replace)
        if u != null_unit: u.add(o)

    def get_o(t, to = '*', autofill = False): return t.get_u(to).get()
...
class order:
    ...
    def __str__(t):
        ...
        for v in t.via:
            res += w + v.occu()
            w = w2
        if DEBUG: return 'order<'+res+'>'
        return res

    def update_status(status, face ='', stage='final', code=0, *cause):
        pass
    ...
    def clear_results(t):
        ...
        t.checked = {'valid':False, 'legal':False, 'match':False, 'path':False, 't2h':False, 'h2h':False}
        t.stage = ''
        t.face = ':/'
        t.status = 'tentative'
    ...
    def fill_relations(t):
        ...
        if t.ord_type in (MOV, DEF): t.at_hdest.add_o(t)
        if t.ord_type == SUP: t.at_hterr.add_o(t)
        for i in t.via: t.add_o(t.via.terr.get_o())
        if t.ord_type != CON and len(t.via) == 0: t.checked['path'] = True
        if t.ord_type not in (SUP, CON) and len(t.via) == 0: t.checked['match'] = True
    ...
    def check_valid(t):
        ...
        if u == null_unit:
            v=f[:-2]+'?'
            t.update_status('canceled', v, 'valid', 415, t.at_hterr)
        if u.ord != t:
            t.update_status('canceled', f, 'legal', 416, u.conflicting)

    def check_legal(t, law):
        if t & law:
            f = t.ut.summary('*', mememode = True)
            t.update_status('canceled', f, 'legal', 417, law)
    ...
    def check_matching_ord(t, inner, outter, ord_type = SUP):
        ...
        for i in range(len(outter)):
            ...
            if inner & outter: 
                t.update_status(yes, ':)', 'match', code=620)
                return
        t.update_status(no, ':(', 'match', 622, t.at_hterr)
